"""
ULTIMATE TRAINER — 5 architectures + XGBoost + Ensemble
"""
...
LOG = os.path.join(MODEL_DIR, 'ultimate_log.txt')
...
log('='*60)
log('ULTIMATE TRAINER: 5 architectures + XGBoost + Ensemble')
log('='*60)
t_start = time.time()
...
try:
    imp_old = joblib.load(os.path.join(MODEL_DIR, 'improved_model.pkl'))
    X_train_s = imp_old['X_train_s']
    X_test_s = imp_old['X_test_s']
    y_train = imp_old['y_train']
    y_test = imp_old['y_test']
    imp = imp_old['imputer']
    scaler = imp_old['scaler']
    FEATURES = imp_old['features']
    NUM_CLASSES = 25
    log(f'Loaded preprocessed data: {len(X_train_s)} train, {len(X_test_s)} test')
    ...
    log(f'Loaded: {len(X_train)} train, {len(X_test)} test, {X_train.shape[1]} features')
...
log(f'Features: {NUM_INPUT}, Classes: {NUM_CLASSES}')
...
results = {
    'xgb': {
        'exact': float(np.mean(xgb_pred == y_test)),
        '1x2': float(np.mean(np.array([_result(*_class_to_score(c)) for c in xgb_pred]) == actual_1x2)),
        'rps': compute_rps(y_test, xgb_proba),
    }
}
log(f'XGBoost: exact={results["xgb"]["exact"]*100:.2f}% 1X2={results["xgb"]["1x2"]*100:.2f}% RPS={results["xgb"]["rps"]:.4f}')

# 2. Multi-architecture DeepNNs
ARCHITECTURES = {
...
for arch_name, layers in ARCHITECTURES.items():
    ...
    model = M5_Variant(NUM_INPUT, NUM_CLASSES, layers).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % 15 == 0:
            ...
            log(f'  ep {epoch+1}/{EPOCHS}: val={acc*100:.2f}% best={best_acc*100:.2f}% rem={rem/60:.0f}m')
    ...
    log(f'  DONE: exact={results[arch_name]["exact"]*100:.2f}% 1X2={results[arch_name]["1x2"]*100:.2f}%')
    del model, train_loader
    gc.collect()

# 3. Ensemble search
log('\n[3/6] Searching ensemble blends...')
...
for nm in model_names:
    r = results[nm]
    log(f'{nm:<20} {r["exact"]*100:>7.2f}% {r["1x2"]*100:>7.2f}% {r["rps"]:>7.4f}')
log('-'*44)
...
try:
    ...
    old_ens_pct = old.get('ensemble', {}).get('exact_pct', 24.69)
    delta = ensemble_exact*100 - old_ens_pct
    log(f'\nImprovement vs improved_model ({old_ens_pct}%): {delta:+.2f}pp')
except:
    pass
...
final = {
    ...
    'individual_results': {nm: {k: float(v) if isinstance(v, np.floating) else v for k, v in results[nm].items()} for nm in model_names},
...
xgb_model.save_model(os.path.join(MODEL_DIR, 'ultimate_xgb.ubj'))

# Save results
json.dump({
    'model': 'ULTIMATE',
    'test_samples': len(X_test_s),
    'individual': {nm: {'exact_pct': round(results[nm]['exact']*100, 2), '1x2_pct': round(results[nm]['1x2']*100, 2), 'rps': round(results[nm]['rps'], 4)} for nm in model_names},
...
log('\n' + '='*60)
log(f'ULTIMATE TRAINER DONE! {total_t/60:.1f} min')
