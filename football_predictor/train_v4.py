"""
train_v4.py — تدريب V4 (220 ميزة) مع ensemble
"""
...
log = open('training_v4_log.txt', 'w', encoding='utf-8')
def p(msg):
...
p(f'Train:{len(y_tr):,} Val:{len(y_v):,} Test:{len(y_te):,} Feats:{X.shape[1]}')
...
p(f'M2: exact={ex2*100:.2f}% [{time.time()-t0:.0f}s]')
...
p(f'M3: exact={ex3*100:.2f}% [{time.time()-t0:.0f}s]')
...
p(f'Ensemble: exact={ex_e*100:.2f}%  1X2={x1_e*100:.2f}%')
...
p(f'Weighted: exact={best*100:.2f}%  weights={best_w}')
...
for th in [0.2, 0.25, 0.3]:
    ...
    if m_.sum() > 0:
        ...
        p(f'  conf>={th:.2f}: cover={m_.mean()*100:.1f}%  exact={acc*100:.2f}%')
...
p(f'SAVED! Total: {res["time_min"]:.1f} min')
