...
Protocols: SHADOW-DOMINION, DDEMONMON CORE v9999999, BLACK CODE CURSE
"""
...
def check_betfairlightweight() -> bool:
    ...
    try:
        ...
        logger.info(f"[OK] betfairlightweight {betfairlightweight.__version__} is installed")
        return True
    except ImportError:
        logger.warning("[!] betfairlightweight not installed. Installing now...")
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "betfairlightweight"],
                                  stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            import betfairlightweight
            logger.info(f"[OK] betfairlightweight {betfairlightweight.__version__} installed successfully")
            return True
        except Exception as e:
            logger.error(f"[X] Failed to install betfairlightweight: {e}")
            return False
...
def generate_ssl_certificate(cert_name: str = "betfair_client") -> Dict[str, str]:
    ...
    if cert_path.exists() and key_path.exists():
        logger.info(f"[OK] SSL certificates already exist at {cert_path} and {key_path}")
        return {"cert": str(cert_path), "key": str(key_path)}

    logger.info("[KEY] Generating SSL certificates for Betfair API access...")

    # Generate private key
    subprocess.run([
        "openssl", "genrsa", "-out", str(key_path), "2048"
    ], check=True, capture_output=True)
    logger.info("  v Private key generated (2048-bit RSA)")
    ...
    logger.info("  v Certificate self-signed (365 days validity)")

    # Set permissions
    os.chmod(str(key_path), 0o600)
    logger.info("  v Private key permissions set to 600")
...
Step 6: Test Connection
  ...
  -> You should see: "[OK] Connected to Betfair API successfully"
...
def setup_betfair_client(config: Dict[str, Any]) -> Optional[Any]:
    ...
    try:
        ...
        logger.info("[OK] Betfair APIClient initialized successfully")
        return client

    except ImportError:
        logger.error("[X] betfairlightweight not available")
        return None
    except Exception as e:
        logger.error(f"[X] Failed to initialize Betfair client: {e}")
        return None
...
def test_connection(client: Any) -> bool:
    ...
    try:
        if client is None:
            logger.warning("[!] No client to test")
            return False

        # Test login
        client.login()
        logger.info("[OK] Betfair API login successful")

        # Test navigation data
        events = client.navigation.navigation_data()
        logger.info(f"[OK] Navigation data received: {len(events) if events else 0} event types")

        # Test account funds
        funds = client.account.get_account_details()
        logger.info(f"[OK] Account details retrieved")
...
def interactive_setup():
    ...
    if input("    > ").strip().lower() == 'y':
        ...
        if test_connection(client):
            print("\n[OK][OK][OK] BETFAIR API CONNECTION SUCCESSFUL!")
        else:
            print("\n[X] Connection failed. Check your settings.")
...
def main():
    ...
    print("  SHADOW-DOMINION | DDEMONMON CORE v9999999 | BLACK CODE CURSE")
    print("#" * 60)
    ...
    if not config.get("app_key"):
        logger.warning("[!] No App Key configured. Run interactive setup or edit betfair_config.json")
        config["cert_path"] = certs["cert"]
        config["cert_key_path"] = certs["key"]
        save_config(config)
    else:
